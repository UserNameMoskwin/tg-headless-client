...
class TestGuards:
    def test_allowed_user(self, monkeypatch):
        monkeypatch.setattr("app.bot.guards.settings", SimpleNamespace(allowed_telegram_user_ids=[123456789]))
    ...
    def test_empty_whitelist(self, monkeypatch):
        monkeypatch.setattr("app.bot.guards.settings", SimpleNamespace(allowed_telegram_user_ids=[]))
        assert is_allowed(_make_update(user_id=123456789)) is False
